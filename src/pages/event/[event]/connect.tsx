...
const ConnectPage = ({ attestator, attestator_cid, event, events }) => {
  ...
  console.log('attestator: ', attestator)
  useEffect(() => {
    if (checkIn) {
  ...
  return (
    <PageContainer>
      ...
      {attestator && (
        <StyledCard>
          <CardTitle>Connect with</CardTitle>
          <StyledLink href={`/profile/${attestator.data.id}`}>
            <UserCard user={attestator.data} variant="vertical" />
          </StyledLink>
        </StyledCard>
      )}
...
export const getServerSideProps = async ({ query, res }) => {
  const attestator = query.attestator
    ? await getProfile(query.attestator)
    : null;
